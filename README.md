...
- 바 색은 사용량에 따라 초록 → 노랑 → 빨강으로 바뀐다 (기본 75% / 90%).
...
- 툴바 버튼·모델 선택기와 겹칠 자리면 그리지 않고 숨는다 (아래 "겹침 회피" 참고).
...
`build.sh`는 첫 실행 시 로컬 자체 서명 인증서(`ClaudeUsageOverlay Local Signing`)를 로그인 키체인에
자동 생성하고, 이후 빌드는 항상 이 인증서로 서명한다. TCC는 권한을 "번들 ID + 인증서"에 묶으므로
코드를 고쳐 재빌드해도 손쉬운 사용 권한이 유지된다. 승인은 맥 한 대당 최초 한 번만 필요하다
...
Claude 앱은 화면 종류에 따라 입력창 툴바 배치가 다르다. **코드** 화면은 가운데가
비어 있어 바가 그 자리에 깔끔하게 들어가지만, **채팅** 화면은 같은 자리에 모델
선택기(`Opus 5 최대`)가 있어 그대로 그리면 글자가 겹친다.

그래서 화면 종류를 판별하는 대신, **그 자리에 실제로 컨트롤이 있는지**를 매번 확인해서
있으면 숨긴다 (접근성 트리에서 툴바의 버튼·라벨 프레임을 읽어 교차 검사). 결과적으로
코드 화면에서는 보이고 채팅 화면에서는 조용히 사라지며, 앱 레이아웃이 바뀌어도 별도
수정 없이 따라간다.
...
바가 아슬아슬하게 붙거나 반대로 너무 잘 숨는다면 `overlapMargin`을 조절한다 (크게 =
더 잘 숨음). 숨는 순간은 로그에 남는다:

```bash
grep -E "hidden|shown" ~/Library/Logs/ClaudeUsageOverlay.log
```

### 바 색상

사용량 구간에 따라 색이 바뀐다. 색값은 Claude 앱의 채우기 토큰을 그대로 쓴다
(`--cds-fill-success/warning/danger`), 그래서 앱 UI와 이질감이 없다:

| 구간 | 색 | 토큰 |
|---|---|---|
| `< warnAt` (기본 75%) | 초록 `#009300` | `--cds-green-450` |
| `>= warnAt` | 노랑 `#fab219` | `--cds-yellow-200` |
| `>= dangerAt` (기본 90%) | 빨강 `#d03b3b` | `--cds-red-450` |

기본 임계값 75%는 임의로 고른 값이 아니라 서버가 내려주는 `severity` 필드에서
역산했다 — 응답에서 76%는 `warning`, 49%와 39%는 `normal`이었으므로 경계는
(49, 76] 안에 있다. 더 일찍/늦게 경고받고 싶으면 `tunables.json`에서 조정한다:
